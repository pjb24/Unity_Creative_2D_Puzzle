...
///
/// 사용법:
/// - 부모(피벗)에 본 스크립트 + GridObject 부착
/// - 자식에 회전 대상 타일/플랫폼 Mesh/Sprite 배치
/// - 탑승 감지용 Trigger Collider2D(부모/자식 아무 곳) 1개 배치 권장
...
public class RotatingFloor : MonoBehaviour
{
    ...
    public void TriggerRotateToAbsoluteRaw(float targetAngleRaw)
    {
        ...
        _elapsed = 0f;
        _rawStartAngle = _rawCurrentAngle;
        _rawTargetAngle = targetAngleRaw; // 예: 현재 270 → 목표 450(=90) 가능
        _rawPrevAngle = _rawStartAngle;
